...
class FSLikelihood(Likelihood):
    ...
    use LPT over EPT
    for galaxy-matter cross spectrum
    """
    zfid: float
    # Hz_fid: float
    # chiz_fid: float
    ...
    def fs_predict(self):
        ...
        pp   = self.provider
        modPT= pp.get_result('pt_pk_ell_mod_real')
        ...
        alp0_mm = pp.get_param('alpha0_mm')
        alp2 = pp.get_param('alpha2')
        sn0  = pp.get_param('SN0_gm') # SN (and SN only) needs new parameters
        sn2  = pp.get_param('SN2_gm')
        # b1   = pp.get_param('c1')
        # b2   = pp.get_param('c2')
        # bs   = pp.get_param('cs')
        ...
        bpoly = compute_biaspoly(bvec,bvec2,ff)
        ...
        kv = modPT.kv
        p0 = np.sum(modPT.p0ktable * bpoly,axis=1)# + sn + 1./3 * kv**2 * sn2 + 1./5 * kv**4 * sn4
        p2 = np.sum(modPT.p2ktable * bpoly,axis=1)# + 2 * kv**2 * sn2 / 3 + 4./7 * kv**4 * sn4
        p4 = np.sum(modPT.p4ktable * bpoly,axis=1)# + 8./35 * kv**4 * sn4
...
class PT_pk_theory_real(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        lpt.make_pltable(ff,kmin=min(ki),kmax=max(ki),nk=len(ki))
        state['pt_pk_ell_mod_real'] = lpt
#         #